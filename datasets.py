...
class BaseCompositionDataset(Dataset):
    def __init__(self, config, phase, split) -> None:
        ...
        self.open_world = config.open_world
    ...
    def generate_features(self, out_file, model):
        ...
        feat_extractor = feat_extractor.to(self.config.device)

        image_feats = []
        image_files = []
        for chunk in tqdm(self.chunks(files_all, 1024), total=len(files_all) // 1024, desc=f'Extracting features {model}'):
            files = chunk
            imgs = list(map(self.loader, files))
            imgs = list(map(transform, imgs))
            feats = feat_extractor(torch.stack(imgs, 0).to(self.config.device))
    ...
    def _set_feats(self, model):
        if not self.config.update_image_features:
            feat_file = os.path.join(self.root, model + '_feature_vectors.t7')
    ...
    def check_mitstates_img(self, datatype, image):
...
class COTDataset(BaseCompositionDataset):
    def __init__(self, config, phase, split, open_world=False) -> None:
        super().__init__(config, phase, split)
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'train':
            ...
            img1 = self.check_mitstates_img(self.config.dataset, img1)
            if self.config.use_precomputed_features:
...
class CANetDataset(BaseCompositionDataset):
    def __init__(self, config, phase, split):
        super().__init__(config, phase, split)
        self.update_image_features = config.update_image_features
        self.feat_dim = 512 if 'resnet18' in config.image_extractor else 2048
        ...
        if config.train_only and self.phase == 'train':
        ...
        self._set_feats(self.config.image_extractor)
...
class IVRDataset(CANetDataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.update_image_features:
            img = self.activations[image]
            if self.phase == 'train':
                same_attr_image = self.check_mitstates_img(self.config.dataset, same_attr_image)
                same_obj_image = self.check_mitstates_img(self.config.dataset, same_obj_image)
    ...
    def same_A_diff_B(self, label_A, label_B, phase='attr'):
        # This method seems not support mit-states dataset
        assert self.config.dataset != 'mit-states'
        data1 = []
...
class CompCosDataset(SCENDataset):
    def __init__(self, config, phase, split):
        super().__init__(config, phase, split)
...
class PROLTDataset(SCENDataset):
    def __init__(
        ...
        if not self.update_image_features:
            ...
            if not os.path.exists(feat_file):
                with torch.no_grad():
                    self._generate_features(feat_file,model,feat_avgpool=False)
            self.phase = phase
            activation_data = torch.load(feat_file)
            self.activations = dict(
                zip(activation_data['files'], activation_data['features']))
    ...
    def _generate_features(self, out_file, model, feat_avgpool=True):
        data = self.train_data + self.val_data + self.test_data
        transform = transform_image('test')
        ...
        for chunk in tqdm.tqdm(
                self.chunks(data, 512), total=len(data) // 512):
            files, attrs, objs = zip(*chunk)
            files = [self.check_mitstates_img(self.config.dataset, file) for file in files]
            imgs = list(map(self.loader, files))
            imgs = list(map(transform, imgs))
            imgs = torch.stack(imgs, 0).cuda()
            if feat_avgpool:
    ...
    def __getitem__(self, index):
        ...
        if self.config.return_images and self.phase != 'train':
            data.append(image)
