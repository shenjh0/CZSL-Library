        ...
        if wordemb == 'glove':
            if cfg.config_name == 'cgqa-czsl':
                wordemb_ = load_word_embeddings(
                    f'{cfg.dataset_path}/../glove/glove.6B.300d.txt', vocab)
            else:
                wordemb_ = load_word_embeddings(
                    f'{cfg.dataset_path}/../glove/glove.6B.300d.txt', vocab)
        elif wordemb == 'word2vec':
            wordemb_ = load_word2vec_embeddings(
                f'{cfg.dataset_path}/../w2v/GoogleNews-vectors-negative300.bin', vocab)
        elif wordemb == 'fasttext':
            wordemb_ = load_fasttext_embeddings(
                f'{cfg.dataset_path}/../fast/cc.en.300.bin', vocab)
        elif wordemb == 'conceptnet':
            wordemb_ = load_word_embeddings(
                f'{cfg.dataset_path}/../conceptnet/mit-states.txt', vocab)
        if result is None:
            result = wordemb_
        else:
