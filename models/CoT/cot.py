...
class COT(nn.Module):
    ...
    def __init__(self, dset, cfg):
        ...
        if not cfg.use_precomputed_features and not cfg.comb_features:
            self.feat_extractor = ViT('B_16', pretrained=True)
        feat_dim = cfg.img_emb_dim
        ...
        self.classifier = CosineClassifier(temp=cfg.cosine_cls_temp) 
        self.attr_classifier = CosineClassifier(temp=cfg.cosine_cls_temp)
        self.obj_classifier = CosineClassifier(temp=cfg.cosine_cls_temp)
    ...
    def _setup_word_composer(self, dset, cfg):
        attr_wordemb, self.word_dim = \
            initialize_wordembedding_matrix(cfg.wordembs, dset.attrs, cfg)
        obj_wordemb, _ = \
            initialize_wordembedding_matrix(cfg.wordembs, dset.objs, cfg)
